...
(function ($) {
    var Notification = function (element, options) {
        ...
        if(this.options.closable) {
            ...
            if ($('.notify-backdrop').length === 0) {
                $('<div class="notify-backdrop fade in"></div>').appendTo('body');
            }
        }
    ...
    Notification.prototype.show = function () {
        if(this.options.fadeOut.enabled) {
            this.$note.delay(this.options.fadeOut.delay || 2000).fadeOut('slow', $.proxy(onClose, this));
        }

        this.$element.empty().append(this.$note);
        this.$note.alert();
    };
